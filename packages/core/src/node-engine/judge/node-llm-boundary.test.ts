...
describe("node LLM boundary", () => {
  it("builds a compact local judge request without final winner fields", () => {
      ...
      agentActions: [
        {
          ...
          agentRole: "entry",
          roleResponsibilities: ["Open the long sector without bypassing the business plan."],
          teamThesisAnchor: "Pressure validates the buyer definition.",
          businessOperatingPrinciple: "Entry actions must convert plan pressure into node evidence.",
          coachNote: "Keep the role split visible to the local judge."
        }
      ]
    });
    ...
    expect(request.instructions.join(" ")).toContain("roleResponsibilities");
  });
