...
当前主线不是直接扩完整 16 队赛事，而是先完成 `Phase 2.0-pre` 的单图 / 定制 BO3 赛事语义校准。

已冻结或暂时冻结的基线：

```text
Phase 1.8：本地真实 LLM BO3 pilot，后续只修阻断性 bug。
Phase 1.9：Phase18 观赛主屏 / 调试控制台，后续只修阻断性 UI bug。
Phase 1.5 real LLM 单图路径：冻结为 CLI-only archive/debug 路径。
Phase 1.7 前台入口：不恢复为当前前台并列主线。
```

当前不建议先做：

```text
不直接扩 16 队 bracket、fixture、赛事调度。
不先做新闻站、奖项站、完整生态页。
不先做远端部署和长期在线任务系统。
不继续扩大 Phase 1.8 / 1.9 的真实 LLM 范围。
不把 Phase 1.9 工具化导播主屏当成最终节目级视觉包装继续深修。
```

Phase 2.0-pre 是否通过，不看命令是否跑完，而看地图命题、队伍唯一方案、攻防语义、裁判诊断、前端表达和定制 BO3 复现是否成立。
...
```text
双方赛前公开输入平等。
经济不裁剪公开输入，只影响 RawOutput -> SubmittedOutput 后进入 Judge 的有效提交。
对手真实经济、买型、当前计划和输出内容不公开。
agent_action 是计划、意图、职责、路线或准备动作，不是 combat ledger。
judge 不能把未落库微观动作写成已经发生的击杀、清点、封锁回防或残局事实。
judge 必须返回完整 diagnostic，供前端、coach timeout 和赛后复盘消费。
新 LLM 调用必须记录 promptContractId、promptHash、taskType、schemaName、request artifact 和 response artifact。
```
...
```text
runtime 只读取 data/materials/processed/
runtime 不直接读取 raw/
runtime 不直接读取整篇 Markdown
机器消费主契约固定为 JSON
Markdown 用于人工审阅、解释和持续维护
```

Phase 2.0-pre 起，队伍方案口径固定为：
...
不要再新增 `strategy.*` 或 `processed/teams/<team-slug>/maps/<map-slug>/initial-proposal.*` 作为平行方案真相。地图只负责命题、裁决、区域语义；队伍只带一份唯一方案进入赛事。
