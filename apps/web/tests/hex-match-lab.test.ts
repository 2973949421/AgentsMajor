...
describe("Hex Match Lab", () => {
  ...
  it("defines a match-first real LLM validation surface", () => {
    ...
    expect(drawer).toContain("按选手折叠的金融决策审计");
    expect(drawer).toContain("允许立场");
    expect(drawer).toContain("必需证据结构");
    expect(drawer).toContain("挑战规则");
    expect(drawer).toContain("Phase0 真实");
    expect(drawer).toContain("结构化立场卡 / 挑战卡");
    expect(drawer).toContain("Phase1+ 局内行动输出");
    expect(drawer).toContain("系统输入材料（折叠，非 agent 输出）");
    ...
    expect(drawer).toContain("isConsumableAgentDigest");
    ...
    expect(drawer).toContain("本局没有可消费的真实 phase0 结构化卡片");
    expect(drawer).toContain("roundStartOutputDigests");
    expect(drawer).toContain("phase0 卡片失败");
    ...
    expect(drawer).toContain("立场方任务");
    expect(drawer).toContain("挑战方任务");
