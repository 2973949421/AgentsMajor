...
describe("node agent action boundary", () => {
  ...
  it("requires role or team business evidence when role context is present", () => {
    const fixture = buildFixture();
    const teamContexts = teamContextsFixture();
    const request = buildNodeAgentActionRequest({ ...fixture, teamContexts });
    const supportAgent = request.agents.find((agent) => agent.agentId === "attack_agent_1")!;
    ...
    const generic = normalizeNodeAgentActionDrafts({
      request,
      rawDrafts: [
        {
          agentId: supportAgent.agentId,
          phaseId: request.phaseId,
          targetNodeId: supportAgent.allowedTargetNodeIds[0],
          actionType: supportAgent.allowedActionTypes[0],
          apCost: 1,
          businessIntent: "Move safely without context."
        }
      ]
    });
    const contextual = normalizeNodeAgentActionDrafts({
      request,
      rawDrafts: [
        {
          agentId: supportAgent.agentId,
          ...
          actionType: supportAgent.allowedActionTypes[0],
          apCost: 1,
          businessIntent: "Support role validates the team thesis and business plan through utility discipline."
        }
      ]
    });

    expect(generic.errors.join(" ")).toContain("missing_role_or_team_business_context");
    expect(contextual.errors).toEqual([]);
    expect(contextual.drafts).toHaveLength(1);
  });
});
