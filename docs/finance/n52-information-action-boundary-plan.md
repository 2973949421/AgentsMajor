...
本轮做 **N52：回合信息层 / 局内行动层硬隔离**。第一版已完成落地。
...
- phase action 输出必须包含 `briefRefId` 或等效引用；缺失或错写时只能修正为当前 agent 自己的信息卡，并记录修复。
- `businessIntent` 作为 legacy 字段时，只能表示本阶段行动理由，不能承载完整金融论文。
- phase 输出重复完整自证 / 质疑时，记录 `phase_repeated_round_thesis` 并拒绝草案。
- phase 行动理由明显超长时，记录 `phase_action_reason_too_long` 并拒绝草案。
...
```text
phase_repeated_round_thesis -> rejected / fallback
phase_action_reason_too_long -> rejected / fallback
repaired_missing_briefRefId -> 修复到当前 agent 的 briefId
repaired_invalid_briefRefId -> 修复到当前 agent 的 briefId
```
