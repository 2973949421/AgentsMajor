# yuurih

## Snapshot
- 队伍：FurIA
- 类型：player
- 角色：rifler
- 当前状态：active
- 公开 ID：yuurih
- 法定姓名：待回填

## CS Role Profile
- 原始位置：Rifler / Support
- 主位置：rifler
- 次级位置：support
- 可信度：中-高
- Agent Major 职责：通用火力 / 回合执行 / 补枪衔接；补全细节 / 修复方案 / 提供上下文
- 备注：团队型步枪与补位。
- 来源：raw/teams/agent_major_player_roles.md

## Alias
- yuurih

## Persona
更偏沉默的暗线处理器，常常负责把前面打出的乱局收成优势。

## Meme Tags
- 暗线补刀

## Playstyle Tags
- rifle-trade
- site-pressure
- timing-lurk
- late-pop

## Broadcast / Barrage / News
- 解说标签：machine_map_point_pressure
- 弹幕标签：player_targeting
- 新闻标签：brazil-heat, old-guard-last-dance, upset-watch
- 跨圈标签：customer_service_to_cleanup

## Future Interfaces
- agent 人格接口：brazil-heat, old-guard-last-dance, upset-watch, rifle-worker, round-connector, quiet-killer
- 模型绑定接口：llm_role_template_rifler / driver_qwen_3_6_plus
- 模型运行状态：v1 asset preallocation only，runtime_enabled=false
- 产品运营接口：暗线补刀, brazil-heat, old-guard-last-dance, upset-watch

## Canon Notes
- 当前以公开比赛 ID 为主，等待后续补 legal name 与更细人物画像。
