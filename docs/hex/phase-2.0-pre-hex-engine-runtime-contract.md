...
N21 起必须采用清晰目录隔离；N34c 后旧 runtime 与旧实验兼容层已经物理删除或退役：
...
- Hex 前端实验台使用 `/hex-lab/*`。
- 旧 `node-graph.*` / `sector-map.*` 运行资产已删除；frozen 文档只作为历史决策记录，不能再作为 runtime 或 Web 验收输入。
...
- N34c：`phase20_node_*` active mode、Node progress/parser/UI 分支、Node shadow sidecar 与 node-sector archive 资产删除；Phase18 replay/live replay 与 Hex 主线保留。
...
Finance Evidence MVP 是 financeDuel 的前置事实层：

```text
collector（采集器）不等于 source（事实来源）。
raw cache 保存原始响应、抓取时间和 hash。
normalized facts 只存标准化事实。
evidence_id 绑定 source、period、metric、unit、rawHash。
round evidence pack 每 round 提供 20-40 条 compact facts。
judge evidence ledger 记录 submittedEvidenceIds、missingEvidence 和 scoreCaps。
```

CNINFO、国家统计局、工信部、SHFE、SMM 等第一版不作为稳定免费自动源；它们只能作为后置证据锚点或商业化替换源。
...
- 不让 FRED / BaoStock / UN Comtrade 的代理事实冒充完整中国有色行业基本面事实。
