...
- Web 主审计展示真实 LLM response artifact 的人工摘要，不用系统输入卡冒充 agent 输出。
...
N49 之后不应立即扩大到新地图。必须先做 N50-N55，把用户准备的免费数据接口、专家证据切片、信息边界、裁判采信、中文人工验收和真实 LLM 输出审计逐层打通。不能再把这些问题压成一个巨大 N。
...
FRED：已生成 offline_observation_fact。
BaoStock：已生成 offline_observation_fact。
UN Comtrade：已生成 optional unavailable_observation。
...
```text
Web 中文人工审计主链路已完成。
humanAudit 新增 roundValidationSummaryZh、phaseValidationSummaryZh 和 sampleQualityWarningsZh。
审计抽屉默认显示中文链路，行动和战斗技术细节折叠。
新跑 real 样本时，沙箱内 provider 请求被 EACCES 阻断。
外部执行申请因真实对局提示词和资产内容会发送给外部 provider，被安全审查拒绝。
因此 N54 不能宣称 real provider 成功样本通过；报告只证明失败路径可审计。
```
...
### N55：真实 LLM 输出人类审计摘要（已完成第一版）

目标：

- 主审计展示 `hex_llm_response` artifact 的人工可读摘要。
- 系统生成的开局信息卡只作为“系统输入卡（非 agent 输出）”折叠展示。
- 没有 response artifact 时明确显示没有真实模型输出，不用系统预置词或 fallback 文案补写。
- 摘要展示行动、理由、风险、证据引用、规范化、校验和裁判采信。
