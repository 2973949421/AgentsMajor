...
完整 Token 经济系统是否还影响激活智能体数量、战术暂停和输出预算，由 P1.2 Token 经济说明定义。Phase 2.0-pre 明确不使用经济裁剪双方共同的公开输入；P1.1 只保存回合结束后实际采用的提交额度。
...
- 裁判依据只能来自 `submittedOutput`、裁判结算层经济状态、地图目标和回合上下文。
