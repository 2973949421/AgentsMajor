...
N49 之后不应立即扩大到新地图。必须先做 N50，把用户准备的免费数据接口转化为低频、可审计、低 token 的金融事实库。
...
- 从事实库生成 R1-R6 round evidence pack。
- 从 round evidence pack 生成 PM / Macro / Commodity / Company / Risk 五类 agent evidence slice。
- 让 10 名选手开局信息卡不再同队复制。
- 让 Web 审计能看到每名 agent 拿到哪些 factId / evidenceId，负责什么问题，哪些证据被裁判采信或降权。

N50 的核心判断：

```text
用户准备 API 的努力没有白费，但当前只到 source registry 和 configured proxy fact。
下一步要先离线生成事实库，再让比赛消费事实库。
