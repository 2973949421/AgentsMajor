...
import { formatBuyType, formatEconomyPosture, type DropDecision, type TeamEconomyPlan } from "../../economy/economy-rules.js";
...
function buildTeamResource(plan: TeamEconomyPlan): NodeTeamEconomyResource {
  return {
    ...
    sharedEconomySummary: `${plan.side === "attack" ? "attack " : "defense "}${formatEconomyPosture(plan.posture)}, team_total_cash ${plan.totalCash}, buy_type ${formatBuyType(plan.summaryBuyType)}. ${plan.postureReason}`,
...
function buildResourceNotes(posture: EconomyPosture, existingNotes: string[], resourceClass: "low" | "medium" | "full"): string[] {
  ...
  if (resourceClass === "low") {
    notes.push(`${formatEconomyPosture(posture)} supports light map control, info gathering, grouping, or saving only; no full execute. AP/node graph still constrain actions.`);
  } else if (resourceClass === "medium") {
    notes.push(`${formatEconomyPosture(posture)} supports local contest and low-config coordination, with explicit resource tradeoffs. AP/node graph still constrain actions.`);
  } else {
    notes.push(`${formatEconomyPosture(posture)} supports default, site execute, retake, and stronger utility coordination. AP/node graph still constrain actions.`);
  }
  return notes;
}
