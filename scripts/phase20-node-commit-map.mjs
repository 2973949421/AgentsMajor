...
import { runDust2NodeMapExperimental } from "../packages/core/dist/node-map-experimental-runner.js";

const args = process.argv.slice(2);
const shouldCommit = args.includes("--commit");
if (!shouldCommit) {
  throw new Error("phase20-node-commit-map requires explicit --commit.");
}

const mapSlug = readArg("--map", "dust2");
if (mapSlug.toLowerCase() !== "dust2") {
  throw new Error(`Only --map dust2 is supported in N16; received ${mapSlug}.`);
}
      ...
      JSON.stringify(
        {
          ...
          committed: true,
          mapGameId,
          status: result.mapGame.status,
          finalScore: result.summary.finalScore,
          roundsCommitted: result.roundsCommitted,
          summaryArtifactId: result.summaryArtifact.id,
          roundTraceArtifactIds: result.summary.roundTraceArtifactIds,
          completionReason: result.summary.completionReason,
