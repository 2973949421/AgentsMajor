...
当前：进入 Phase 2.0 完整 16 队赛事雏形与生态扩展。
```
...
因此，当前应继续保持“近期写深，远期写边界”的文档策略。P2.2 已完成，P2.3 已完成并可作为 Phase 1 转播边界执行，Phase 1.45 已把关键契约落到代码锚点，Phase 1.5 已完成真实 caster_line 小范围接入，Phase 1.6 已完成 deterministic rule-based 区域化攻防协议，Phase 1.8 / 1.9 已作为真实 LLM pilot 与观赛主屏基线冻结。下一步应先转入 Phase 2.0 的边界设计与阶段编号确认，优先确认 16 队 bracket、fixture、赛事调度、失败恢复、公开导出 / API 边界，以及后续 Web / 队列迁移的约束。Phase 3 赛事生态和 Phase 4 Web 化仍只保留边界意识，不阻塞当前主线。
