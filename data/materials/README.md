...
- `processed/maps/`：地图级正式资产库，存放单图命题、裁判规程、区域语义等可被比赛直接消费的地图资料。
- `processed/teams/<team-slug>/maps/<map-slug>/`：队伍在特定地图上的初始方案稿、图后修正版等队伍侧地图资产。
...
- 地图命题、裁判规程、区域语义等公共地图资产，统一沉淀在 `processed/maps/`。
- 队伍进入具体地图时的初始方案、修正版和地图特化备注，统一沉淀在各队 `maps/<map-slug>/` 子目录。
