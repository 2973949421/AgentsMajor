...
import { getAdjacentNodes, getReachableNodes } from "../graph/node-graph-service.js";
...
export function materializeNodeStateTransition(input: MaterializeNodeStateTransitionInput): NodeStateTransitionResult {
  ...
  const nodeStates = activeNodeIds.map((nodeId) => {
    ...
    return {
      ...
      beforeStateSummary: carriedControlByNode[nodeId] ? `上一阶段局部裁定控制权�?${carriedControlByNode[nodeId]}。` : undefined,
      afterStateSummary: `物化�?${input.nextPhaseId} 后，节点控制权为 ${control}。`,
  ...
  notes.push(`materialized ${input.nextPhaseId} node state from previous phase actions and local verdicts.`);
...
function buildBusinessIntent(phaseId: RoundPhaseId, nodeId: string, control: MapNodeControl, carriedControl: MapNodeControl | undefined): string {
  ...
  return `${carryText}�?${phaseId} �?${nodeId} 物化�?${control} 状态，用于后续局部裁判和硬胜负条件检查。`;
}
