...
Phase 1.7 后，active agents 的角色排序必须读取 `Agent.role` / `secondaryRoles`，不能用 agent id 正则猜测角色。
...
`closer` 旧数据读取时映射为 `rifler`，如果需要表达残局特质，应放入 secondaryRoles。
Phase 1.7 tactical protocol 的 role modifier 只能从 Agent.role / secondaryRoles 计算。
```
...
## Phase 1.7 增量：Materials runtime seed 已接入

Phase 1.7 在不改变比赛状态机的前提下，把 `data/materials/processed` 作为 runtime seeding 输入：

```text
默认创建 Falcon-7B vs VitaLLMty BO3。
默认地图为 DUST2 / INFERNO / MIRAGE。
所有 runtime Agent.driverModelId 使用 driver_fake_phase17。
future LLM binding 只作为 materialRef 保存，runtimeEnabled=false。
```

边界：

```text
Phase 1.7 不启用真实 agent / judge LLM。
Phase 1.7 不跑完整 16 队 bracket。
Phase 1.7 不让 materials binding 影响胜负、战术或生成流程。
Phase 1.7 的 seeding 失败应 fail fast，不能静默退回旧 demo 阵容。
```
