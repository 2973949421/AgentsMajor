...
export const PHASE20_PRE_PROMPT_CONTRACT_ID = "phase20pre-prompt-contract-v3";
  ...
  return [
    {
      ...
      content: [
        ...
        "双方公开输入必须平等；经济不裁剪公开输入，只能影响未来 Output Gate 后提交给 Judge 的 SubmittedOutput。",
...
export function buildPhase20PreSchemaContract(schemaName: string): string {
  ...
  if (schemaName === "TeamRoundPlanDecision") {
    return [
      "JSON 输出契约：只返回一个顶层对象，字段包括 teamId、side、primaryIntent、primaryZoneId、可选 secondaryZoneId、coordinationSummary、playerDirectives、winCondition、risk、confidence、可选 fingerprint。",
  ...
  if (schemaName === "JudgeResult") {
    return [
      "JSON 输出契约：只返回一个顶层对象，字段包括 winnerTeamId、loserTeamId、margin、roundWinType、attackWinConditionMet、defenseWinConditionMet、reason、mvpAgentId、confidence、judgeInference、diagnostic。",
      ...
      "Judge 可以在结算层读取双方真实经济与双方完整 agent_action，但不得引用 RawOutput 中未提交给自己的内容或被未来 Output Gate 裁掉的内容。",
...
export function buildPhase20PreTaskInstruction(task: Phase20PrePromptTask): string {
  switch (task) {
    ...
    case "judge":
      return [
        "任务说明：根据双方 proposal summary、双方 teamPlan、10 名选手完整 agent_action、地图命题与裁判规程判定本回合。",
        "先判断双方计划分别想证明什么，再判断选手行动是否执行了证明路径，再判断 CS 胜利方式，最后判断商业攻防命题更支持哪一方。",
