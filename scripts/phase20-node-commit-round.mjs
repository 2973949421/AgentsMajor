...
import { commitDust2NodeRoundExperimental } from "../packages/core/dist/node-round-experimental-committer.js";

const args = process.argv.slice(2);
const shouldCommit = args.includes("--commit");
if (!shouldCommit) {
  throw new Error("phase20-node-commit-round requires explicit --commit.");
}

const mapSlug = readArg("--map", "dust2");
if (mapSlug.toLowerCase() !== "dust2") {
  throw new Error(`Only --map dust2 is supported in N15; received ${mapSlug}.`);
}

const providerMode = readArg("--provider", "fixture") === "real" ? "real" : "fixture";
const mapGameIdArg = readArg("--map-game-id", undefined);
const localJudgeLlmShadow = args.includes("--local-judge-llm-shadow");
const agentActionLlmShadow = args.includes("--agent-action-llm-shadow");
const maxLocalJudgeLlmCalls = parsePositiveInt(readArg("--max-local-judge-llm-calls", undefined), 5);
const maxAgentActionLlmCalls = parsePositiveInt(readArg("--max-agent-action-llm-calls", undefined), 5);
...
async function main() {
  ...
  try {
    ...
    const result = await commitDust2NodeRoundExperimental({
      ...
      localJudgeLlmShadow,
      localJudgeProviderMode: providerMode,
      maxLocalJudgeLlmCalls,
      agentActionLlmShadow,
      agentActionProviderMode: providerMode,
      ...
      JSON.stringify(
        {
          ...
          winnerTeamId: result.round.winnerTeamId,
          ...
          nodeTraceSource: result.roundReport.nodeTraceSource,
          source: result.committedReport.source,
          writesDb: result.committedReport.audit.writesDb,
          replacesLegacyRoundPath: result.committedReport.audit.replacesLegacyRoundPath,
          llmShadowEnabled: result.committedReport.audit.llmShadowEnabled,
          agentActionLlmEnabled: result.committedReport.audit.agentActionLlmEnabled,
          finalWinCondition: result.committedReport.finalWinCondition
        },
        null,
        2
      )
