  ...
  return (request) => ({
    ...
    rawDrafts: request.agents.map(
      (agent): NodeAgentActionDraft => ({
        ...
        businessIntent: `${agent.teamId} �?${agent.agentId} 通过 fixture LLM action 继续验证 ${agent.baselineTargetNodeId} 的商业计划执行质量。`,
  ...
  return [
    {
      role: "system",
      content:
        "你是节点�?CS x 商业计划对抗的阶段行动编排助手。只输出 JSON object，形状为 {\"drafts\":[...]}。不得输�?final winner、roundWinType、kill ledger、DB fact、经济参数修改、未�?nodeId�?"
    },
    {
      role: "user",
          ...
          drafts: [
            {
              ...
              targetNodeId: "<�?agent allowedTargetNodeIds 中的节点>",
              actionType: "<�?agent allowedActionTypes 中的行动>",
              apCost: 1,
              businessIntent: "<一句中文，必须说明 CS 行动如何验证商业计划/分工>",
              tacticalIntent: "<可选，短战术意�?",
              riskNotes: ["<可选，短风险说�?"],
              confidence: 0.5
            }
          ]
