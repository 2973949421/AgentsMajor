...
Token 预算如何体现为不同区域的文本火力、上下文可见度和 agent 输出投入？
...
Judge 只看 SubmittedOutput、战术摘要、经济状态和公开上下文，不看未提交 rawOutput。
...
6. 根据信息边界构建双方 Prompt Context。
7. LLM Driver 生成 RawOutput。
8. Output Gate 裁剪出 SubmittedOutput。
9. Judge 根据 SubmittedOutput、战术摘要、经济状态和上下文判定 TacticalCollision。
...
```text
攻方可见：自身经济、自身 AttackPlan、公开比分、公开地图摘要、近期公开事件。
守方可见：自身经济、自身 DefenseDeployment、公开比分、公开地图摘要、近期公开事件。
攻方不可见：守方完整区域部署。
守方不可见：攻方真实主攻点和假打意图。
Judge 可见：双方 SubmittedOutput、双方战术摘要、经济状态、公开上下文。
观众可见：回合事实写入后的战术结果，不提前剧透隐藏计划。
```
