...
- 这些文档已经覆盖事实源、事件、规则、回合战报、经济、驾驶员、比赛推进流程、本地持久化、直播时间线和 2D 战术地图消费契约。
...
- P2.2 已经明确 2D 战术地图如何消费结构化事实源；下一层风险转向转播系统：哪些内容来自事实源，哪些只是可延后、可丢弃的包装层。
...
- 这个工程切换不取消 P2.3、P3、P4。
- P2.2 已经补齐；P2.3 可以在 Phase 1.4 之后继续补文档和实现。
...
因此，当前应继续保持“近期写深，远期写边界”的文档策略。P2.2 已经完成，下一步应补 P2.3 转播系统说明；Phase 2 完整赛事、Phase 3 赛事生态和 Phase 4 Web 化只保留边界意识，不阻塞当前主线。

### 后续文档补充顺序

```text
已完成：docs/p2-broadcast-viewer/tactical-map.md
当前：docs/p2-broadcast-viewer/broadcast-system.md
```
