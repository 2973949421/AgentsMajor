...
import { buildNodeTeamContexts } from "../team-context/node-team-context-service.js";
import {
...
import { buildNodeRoundCommittedReport, type NodeRoundCommittedReport } from "../report/node-round-report-bridge.js";
...
import type { NodeRoundWinConditionResult, NodeRoundWinType } from "../state/win-condition-materializer.js";
...
async function commitDust2NodeRoundExperimentalInner(
  ...
  const teamContexts = buildNodeTeamContexts({
    teams: [teamA, teamB],
    agents: [...activeA, ...activeB]
  });
...
function buildRoundReport(input: {
  ...
  const judgeResult: JudgeResult = {
    ...
    judgeInference: {
      ...
      combatNarrative: "节点�?experimental commit 只桥接节点行动和硬胜负条件，不伪造击杀回放�?",
      evidenceBasis: input.finalWinCondition.evidence
    },
    diagnostic: {
      currentSubTheme: "节点化实验回�?",
  ...
  return {
    ...
    tokenSubmission: {
      ...
      outputGate: {
        applied: false,
        reason: "节点�?experimental commit 使用 node trace 作为审计来源，不运行�?SubmittedOutput Gate�?",
        teamPostures
      }
    },
    ...
    summary: `节点化实验回�?R${input.roundNumber}: ${input.finalWinCondition.reason}`,
    eventProjection: {
  ...
  return agents.map((agent) => {
    ...
    return {
      ...
      actionDetail: {
        ...
        roleResponsibilityUsage: action?.roleResponsibilities?.[0] ?? `${agent.role} follows node action type ${action?.actionType ?? "hold_position"}.`,
