    ...
    ({
      ...
      rawDrafts: request.nodes.map(
        (node): LocalNodeJudgeDraft => ({
          ...
          summary: `fixture LLM shadow: ${node.nodeId} �?${request.phaseId} 的商业碰撞被局部解释。`,
  ...
  return [
    {
      role: "system",
      content:
        "你是节点�?CS x 商业计划对抗的局部裁判。只输出一�?JSON object，形状为 {\"drafts\":[...]}。不得输�?final winner、roundWinType、kill ledger、DB fact、经济参数修改、未�?nodeId�?"
    },
    {
      role: "user",
