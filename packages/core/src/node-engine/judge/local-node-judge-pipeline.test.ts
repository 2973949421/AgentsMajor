...
describe("local node judge pipeline", () => {
  ...
  it("carries role, thesis, and coach context into local verdict evidence", () => {
    const graph = loadMapNodeGraph("dust2");
    const enrichedActions = actions().map((action) =>
      action.agentId === "attack_agent_1"
        ? {
            ...action,
            agentRole: "igl",
            roleResponsibilities: ["Call the mid control plan."],
            teamThesisAnchor: "Mid control proves the buyer definition.",
            businessOperatingPrinciple: "IGL converts map pressure into business plan evidence.",
            coachNote: "Coach checks whether role discipline survived contact."
          }
        : action
    );
    const verdict = buildLocalNodeVerdicts({
      graph,
      phaseSnapshot: snapshot(),
      agentActions: enrichedActions
    }).find((item) => item.nodeId === "mid")!;
    const evidence = verdict.businessPlanValidated?.join(" ") ?? "";

    expect(evidence).toContain("role:igl");
    expect(evidence).toContain("team_thesis:Mid control proves the buyer definition.");
    expect(evidence).toContain("coach_note:Coach checks whether role discipline survived contact.");
  ...
  return [
    {
      ...
      apCost: 1,
      businessIntent: "用中路行动验�?buyer 定义是否成立�?",
